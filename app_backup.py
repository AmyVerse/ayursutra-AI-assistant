...
async def get_ai_response(message: str, search_medicines: bool = False) -> ChatResponse:
    ...
    try:
        if google_api_key and model:
            ...
            # Use the response_length parameter passed to the function
            length_configs = {
                "short": {
                    "word_range": "50-120 words",
                    "max_tokens": 200,
                    "temperature": 0.5,
                    "instructions": "Be very concise. 1-2 key points only."
                },
                "medium": {
                    "word_range": "120-250 words", 
                    "max_tokens": 350,
                    "temperature": 0.7,
                    "instructions": "Be helpful but concise. 2-3 key points."
                },
                "long": {
                    "word_range": "250-400 words",
                    "max_tokens": 550,
                    "temperature": 0.8,
            ...
            config = length_configs[response_length]
            
            # Analyze if this is a health-related query that needs medical advice
            health_keywords = ['pain', 'ache', 'sick', 'disease', 'cure', 'medicine', 'remedy', 'treatment', 'symptom', 'health', 'ayurveda', 'herbal', 'dosage', 'tablet', 'capsule', 'syrup', 'oil', 'powder', 'churna', 'vati', 'ras', 'asava', 'aristha']
            is_health_query = any(keyword in message.lower() for keyword in health_keywords)
            
            if is_health_query:
                # Health-related query - provide medical advice
                full_prompt = f"""You are an expert Ayurvedic health assistant. The user has a specific health concern.

User's health concern: {message}
Context: {context}

IMPORTANT: Keep your response to {config['word_range']}. {config['instructions']}

Since this is a health-related query, please provide:
1. Understanding of their specific health concern
2. Relevant Ayurvedic remedies and treatments
3. Specific medicine recommendations with dosage if applicable
4. Lifestyle suggestions (diet, yoga, daily routine)
5. When to consult a qualified Ayurvedic practitioner

Format: Use bullet points. Be practical and specific about remedies.
"""
            else:
                # General conversation - no medical advice
                full_prompt = f"""You are a friendly AI assistant with knowledge of Ayurveda and wellness.
...
This appears to be general conversation, not a specific health concern. Please:
1. Respond naturally to their message
2. If relevant, share general wellness tips or Ayurvedic lifestyle principles
3. DO NOT recommend specific medicines unless they specifically ask about a health issue
4. Keep the conversation friendly and informative

Format: Be conversational. Only mention Ayurveda if naturally relevant to the topic.
"""
            ...
            generation_config = genai.types.GenerationConfig(
                ...
                top_k=20,         # Lower top_k for faster token selection
            ...
            # Only extract medicines for health-related queries
            suggested_medicines = []
            if is_health_query:
                suggested_medicines = extract_medicine_suggestions(ai_response)
                print(f"🔍 Extracted medicines: {suggested_medicines}")
            else:
                print("💬 General conversation - no medicine extraction")
            
            return ChatResponse(
                response=ai_response,
                suggested_medicines=suggested_medicines if suggested_medicines else None,
